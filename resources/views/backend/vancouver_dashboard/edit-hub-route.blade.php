...
@section('title', 'Vancouver Route Hub ')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            function getDataOrderFlagHistory(data) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{URL::to('/vancouver/route-details/flag-history-model-html-render')}}",
                    data:parse_data,
...
@section('content')
  ...
  <script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script> -->
        ...
        <div class="">
            ...
            <div class="row">
                ...
                <div class="col-md-12 col-sm-12 col-xs-12">
                    ...
                    <div class="x_panel">
                        <div class="x_title">
                            <h2>Toronto <small>Route Detail</small></h2>
                            <div class="clearfix"></div>
                        </div>
                        ...
                        <div class="x_content">
                            ...
                            <div class="table-responsive">
                                <table id="datatable" class="table table-striped table-bordered">
                                    ...
                                    $unflagedBy = \App\FlagHistory::unflagedBy($routeLoc->tracking_id, $routeLoc->sprint_id);
    ...
    height: 80px;" alt="'.$routeLoc->tracking_id.'"/>';
                                    ...
                                    echo "<td>"?>
                                    @if(can_access_route('vancouver-route-info.getNotes',$userPermissoins))
                                        @if ($joey != null)
                                        ...
                                        @endif
                                    @endif
                                    @if(can_access_route('vancouverinfo_route.detail',$userPermissoins))
                                        <a href="{{backend_url('vancouver/route/orders/trackingid/'.$routeLoc->sprint_id.'/details')}}" target='_blank' class='btn btn-warning btn-xs'>Detail</a>
                                    @endif
									@if(isset($routeLoc->SprintTask->sprint->Vendor))
                                        @if($routeLoc->SprintTask->sprint->Vendor->type == 'ecommerce')

                                            @if(empty($unflagedBy))
                                                <button
                                                        data-flag_values='{
												"order_type":"{{$routeLoc->SprintTask->sprint->Vendor->type}}",
												"tracking_id":"{{$routeLoc->tracking_id}}",
												"joey_id":"0",
												"route_id":"{{$routeLoc->route_id}}",
												"sprint":"{{$routeLoc->sprint_id}}",
												"flag_type":"order",
												"vendor_id":"{{$routeLoc->creator_id}}",
												"hub_id":"{{isset($routeLoc->joeyRoute) ? $routeLoc->joeyRoute->hub : ''}}"
												}'
                                            ...
                                            @else
												...
												"hub_id":"{{isset($routeLoc->joeyRoute) ? $routeLoc->joeyRoute->hub : ''}}"
                                                        ...
                                                        class='btn btn-info btn-xs createFlag' >
                                                    Flagged
                                                </button>
                                            @endif
									@endif
    ...
    <script>
        function showAddNoteModal(t_id,r_id) {
            ...
            $.ajax({
                type:'GET',
                url:'{{route("vancouver-route-info.getNotes")}}',
        ...
        function sendNote() {
            if($('#myNotesModal #note').val()==''){
                ...
                $.ajax({
                    type:'POST',
                    url:'{{route("vancouver-route-info.addNote")}}',
                    data: formData,
                    // contentType: false,
                    // processData: false,
                    success: function (res) {
