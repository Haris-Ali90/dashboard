...
@section('title', 'Toronto Route Info')
        ...
        /*  $('.buttons-excel').on('click', function (event) {
              ...
              $.ajax({
                ...
                success: function (data) {
                    ...
                    link.setAttribute("download", "Toronto-route-info-" + selected_date + ".csv");
...
*/
        ...
        $(document).on('click', '.order-delivered-permission', function (e) {
            let el = $(this);
            let route_id = el.attr("data-route-id");

            $.confirm({
                title: 'A secure action',
                content: 'Are You Sure Want To Dispatch?',
                icon: 'fa fa-question-circle',
                animation: 'scale',
                closeAnimation: 'scale',
                opacity: 0.5,
                buttons: {
                    'confirm': {
                        ...
                        action: function () {
                            ...
                            $.ajax({
                                type: "GET",
                                url: "{{URL::to('/')}}/toronto/route/delivered_permission/" + route_id,
        ...
        $(document).on('click', '.order-delivered-denied', function (e) {
            let el = $(this);
            let route_id = el.attr("data-route-id");

            $.confirm({
                title: 'A secure action',
                content: 'Are You Sure Want To Un-Dispatch?',
                icon: 'fa fa-question-circle',
                animation: 'scale',
                closeAnimation: 'scale',
                opacity: 0.5,
                buttons: {
                    'confirm': {
                        ...
                        action: function () {
                            ...
                            $.ajax({
                                type: "GET",
                                url: "{{URL::to('/')}}/toronto/route/delivered_permission_denied/" + route_id,
                                success: function (res) {
                                    hideLoader(res);
        ...
        <div class="">
            <div class="page-title">
                <div class="title_left amazon-text">
                    <!-- <h3 class="text-center"> Toronto Route Info<small></small></h3> -->
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <div class="x_panel">
                        <div class="x_title">
                           <div class="col-lg-6">
                           <h2>Toronto Route Info <small></small></h2>
                           </div>
                           <div class="col-lg-6">
                           @if(can_access_route('export_borderlessRouteInfo.excel',$userPermissoins))
                                <div class="excel-btn" style="float: right">
                                    <button id="btnExport"
                                            class="btn btn-circle buttons-excel buttons-html5 btn-sm btn-primary excelstyleclass c-btn">
                                        Export to Excel
                                    </button>
                                </div>
                            @endif
                            @if(can_access_route('total-order.notinroute',$userPermissoins))
                                <div class="excel-btn" style="float: right">
                                    <button id="order-count-button" class="btn btn-circle c-btn btn-sm btn-primary totalOrderCount" data-id='17'>
                                        Order Count
                                    </button>
                                </div>
                            @endif
                        ...
                        <div class="x_title">
                            <form method="get" action="" class="d-flex align-items-center">
                               ...
                               <button class="btn btn-primary c-btn " type="submit" style="margin: 20px 0px;">
                                    Go</a> </button>
                               </div>
                               </div>
                            </form>
                            ...
                            <div class="table-responsive">
                                <table id="datatable" class="table table-striped table-bordered">
                                    ...
                                    <tr>
                                        ...
                                        <th class="text-center ">Last Delivered Time</th>
                                    ...
                                    @foreach( $boradless_info as $record )
                                        <tr>
                                            ...
                                            <td class="@if($record->TotalSortedOrdersCount() === $record->TotalOrderPickedCount())  @else label-danger @endif">{{$record->TotalSortedOrdersCount()}}</td>
                                            @php
                                                $delivered = $record->getLastActivityTime($record->id);
                                                $updatedAt = strtotime($delivered);
                                                $currentDateTime = strtotime(date('Y-m-d H:i:s'));
                                                $diffCurrentAndUpdatedAt = $currentDateTime - $updatedAt;
                                                $updatedAtToMin = $diffCurrentAndUpdatedAt/60;
                                                $tdStyle = '';
                                                if($updatedAtToMin > 15 && $updatedAtToMin < 30){
                                                    $tdStyle = 'background-color:yellow;';
                                                }
                                                elseif($updatedAtToMin > 30){
                                                    $tdStyle='background-color:red; color:white';
                                                }
                                                if($delivered != null){
                                                    if($record->TotalOrderPickedCount() == $record->TotalOrderDropsCompletedCount() + $record->TotalOrderReturnCount()){
                                                       $tdStyle='background-color:none; color:#73879C';
                                                   }
                                                }
                                            @endphp

                                            <td style="{{ $tdStyle }}">
                                                {{$record->TotalOrderPickedCount()}}
{{--                                                <p>{{ $updatedAtToMin }}</p>--}}
                                            </td>
                                            <td>{{$record->TotalOrderDropsCompletedCount()}}</td>
                                            <td>{{$record->TotalOrderReturnCount()}}</td>
                                            <td>{{$record->TotalOrderNotScanCount()}}</td>
                                            <td>{{$data = $record->TotalOrderUnattemptedCount()}}</td>
                                            @php
                                                $delivered = $record->getLastDeliveredTime($record->id);
                                                $deliveredTime = strtotime($delivered);
                                                $currentDateTime = strtotime(date('Y-m-d H:i:s'));
                                                $diffCurrentAndDeliveredTime = $currentDateTime - $deliveredTime;
                                                $deliveredTimeToMin = $diffCurrentAndDeliveredTime/60;
                                                $deliveredStyle='';
                                                if($delivered == null){
                                                    $deliveredStyle='background-color:none; color:black';
                                                }else{
                                                   if($deliveredTimeToMin > 30){
                                                        $deliveredStyle='background-color:red; color:white';
                                                   }
                                                   if($record->TotalOrderPickedCount() == $record->TotalOrderDropsCompletedCount() + $record->TotalOrderReturnCount()){
                                                       $deliveredStyle='background-color:none; color:#73879C';
                                                   }
                                                }
                                            @endphp
                                            <td style="{{ $deliveredStyle }}">
                                                {{ConvertTimeZone($delivered,'UTC','America/Toronto')}}
                                                {{--{{ $delivered }}--}}
{{--                                                <p>{{ $deliveredTimeToMin }}</p>--}}
                                            </td>
                                            ...
                                            <td id="actiontab">
                                                @if(can_access_route('borderless_route.detail',$userPermissoins))
                                                    <a href="{{backend_url('toronto/route/'.$record->id.'/edit/hub/17')}}"
                                                       title="Route Details" target='_blank'
                                                       class="btn btn-warning btn-xs" style="float: left;">Route Details
                                                    </a>
                                                @endif
                                                @if($record->is_delivered == 0)
                                                    @if(can_access_route('borderless_route.delivered_permission',$userPermissoins))
                                                        <button class='order-delivered-permission  btn btn-danger btn-xs'
                                                                data-route-id="{{ $record->id }}" title='Order Delivered Permission'>
                                                            Dispatch
                                                        </button>
                                                ...
                                                @else
                                                    @if(can_access_route('borderless_route.delivered_permission',$userPermissoins))
                                                        <button class='order-delivered-denied  btn btn-warning btn-xs'
                                                                data-route-id="{{ $record->id }}" title='Order Delivered Denied'>
                                                            Un-Dispatch
                                                        </button>
                                                ...
                                                <button type='button' class='orange-gradient btn btn-xs @if($cmap_data == '[]')hidden @else enabled @endif' data-toggle='modal' data-target='#ex5' onclick='currentMap({{$route_id}})' title='Map of Current Route' id="rmap">CMap</button>
        ...
        <div class='modal-dialog'>
            ...
            <div class='modal-content'>
                ...
                <div class="col-md-12 text-center" style="margin-top: 5px;">
                    <a href="{{ route('not.routed.orders.list', 17) }}">
                        <button class="btn btn-circle btn-sm btn-primary">
                            Not Routed Orders List
                        </button>
                    </a>
                </div>
            </div>
        ...
        function currentMap(id) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    //     $('#ex5').addClass('hidden');
                    //     $('#rmap').addClass('hidden');
                    //     $('#rmap').removeClass('enabled');
                    //     $('.modal-backdrop').hide();
                    //
                    // }
                    // else{
                    //     $('#ex5').removeClass('hidden');
                    //     $('#rmap').removeClass('hidden');
                    //     $('#rmap').addClass('enabled');
                    //     $('#ex5 .route-id').text("R-" + id);
                    //     mapCreated(data);
                    // }
                    $('#ex5 .route-id').text("R-" + id);
