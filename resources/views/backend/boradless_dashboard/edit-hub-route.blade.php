    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            function getDataOrderFlagHistory(data) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{URL::to('/toronto/route-details/flag-history-model-html-render')}}",
                    data:parse_data,
  ...
  <script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script> -->
    ...
    height: 80px;" alt="'.$routeLoc->tracking_id.'"/>';
                                    ...
                                    @if(can_access_route('borderlessinfo_route.detail',$userPermissoins))
                                        <a href="{{backend_url('toronto/route/orders/trackingid/'.$routeLoc->sprint_id.'/details')}}" target='_blank' class='btn btn-warning btn-xs'>Detail</a>
                                    @endif
									@if(isset($routeLoc->SprintTask->sprint->Vendor))
                                        @if($routeLoc->SprintTask->sprint->Vendor->type == 'ecommerce')
                                            @if(empty($unflagedBy))
                                                <button data-flag_values='{
												...
												}' class='btn btn-warning btn-xs createFlag' >Mark Flag
                                                </button>
                                            @else
                                                <button data-flag_values='{
													"order_type":"{{$routeLoc->SprintTask->sprint->Vendor->type}}",
												    ...
												    "vendor_id":"{{$routeLoc->creator_id}}",
												    "hub_id":"{{isset($routeLoc->joeyRoute) ? $routeLoc->joeyRoute->hub : ''}}"
                                                }' class='btn btn-info btn-xs createFlag' >Flagged
                                                </button>
									        @endif
                                        @endif
                                    @endif
