...
@section('title', 'New York Route Hub ')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            function getDataOrderFlagHistory(data) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{URL::to('/newyork-dashboard/route-details/flag-history-model-html-render')}}",
                    data:parse_data,
...
@section('content')
  ...
  <script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script> -->
    ...
    height: 80px;" alt="'.$routeLoc->tracking_id.'"/>';
                                    ...
                                    echo "<td>"?>
                                    @if(can_access_route('newyork-route-info.getNotes',$userPermissoins))
                                        @if ($joey != null)
                                        ...
                                        @endif
                                    @endif
                                    @if(can_access_route('newyork-dashboard-info_route.detail',$userPermissoins))
                                        <a href="{{backend_url('newyork-dashboard/route/orders/trackingid/'.$routeLoc->sprint_id.'/details')}}" target='_blank' class='btn btn-warning btn-xs'>Detail</a>
                                    @endif
    ...
    <script>
        function showAddNoteModal(t_id,r_id) {
            ...
            $.ajax({
                type:'GET',
                url:'{{route("newyork-route-info.getNotes")}}',
        ...
        function sendNote() {
            if($('#myNotesModal #note').val()==''){
                ...
                $.ajax({
                    type:'POST',
                    url:'{{route("newyork-route-info.addNote")}}',
                    data: formData,
                    // contentType: false,
                    // processData: false,
                    success: function (res) {
