...
class VancouverController extends BackendController
{
    ...
    /**
     * Get Vancouver Dashboard
     */
    public function getVancouverDashboard(Request $request)
    {
        ...
        return backend_view('vancouver_dashboard.vancouver_dashboard', compact('status_code', 'city', 'selectVendor','selectStatus'));
    }

    /**
     * Yajra call after  Vancouver Dashboard
     */
    public function getVancouverDashboardData(Datatables $datatables, Request $request)
    {
        //dd($request->status);
        $sprintId = 0;
        ...
        $start_dt = new DateTime($today_date . " 00:00:00", new DateTimezone('America/Vancouver'));
        $start_dt->setTimeZone(new DateTimezone('UTC'));
        $start = $start_dt->format('Y-m-d H:i:s');

        $end_dt = new DateTime($today_date . " 23:59:59", new DateTimezone('America/Vancouver'));
        ...
        $vancouver = [477607, 477609, 477589, 477641];
        $shipHero = [477613];
        $allVendors = array_merge($vancouver, $shipHero);
        if (!empty($request->get('store_name')) || $request->store_name != "") {

            if ($request->get('store_name') == 'walmart_vendors') {
                $query = $query->whereIn('creator_id', $vancouver);
        ...
        return $datatables->eloquent($query)->setRowId(static function ($record) {
            ...
            if ($record->task_status_id == 17) {
                $preStatus = \App\SprintTaskHistory::where('sprint_id', '=', $record->sprint_id)->where('status_id', '!=', '17')->orderBy('id', 'desc')->first();
                if (!empty($preStatus)) {
            ...
            if (str_contains($record->tracking_id, 'old_')) {
                ...
                return $record->tracking_id;
                    ...
                    if ($record->eta_time){
                        ...
                        $eta_time->setTimeZone(new \DateTimeZone('America/Vancouver'));
    ...
    public function get_trackingorderdetails($sprintId)
    {
        ...
        foreach ($result as $tasks) {
            ...
            $taskHistory = TaskHistory::where('sprint_id', '=', $tasks->sprint_id)->WhereNotIn('status_id', [17, 38])->orderBy('date')
                //->where('active','=',1)
                ->get(['status_id', \DB::raw("CONVERT_TZ(created_at,'UTC','America/Vancouver') as created_at")]);
    ...
    /**
     * Get Vancouver Dashboard Excel Report
     */
    public function vancouverDashboardExcel($date = null, $vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');

        }
        $allVendors = [477607, 477609, 477613, 477589, 477641];
        if ($vendor == 'all-vendors') {
            $vancouver = $allVendors;
        }
        ...
        $file_name = "Vancouver Tracking File " . $file_name . ".csv";

        $start_dt = new DateTime($date . " 00:00:00", new DateTimezone('America/Vancouver'));
        $start_dt->setTimeZone(new DateTimezone('UTC'));
        $start = $start_dt->format('Y-m-d H:i:s');

        $end_dt = new DateTime($date . " 23:59:59", new DateTimezone('America/Vancouver'));
        $end_dt->setTimeZone(new DateTimezone('UTC'));
        $end = $end_dt->format('Y-m-d H:i:s');


        $boradless_data = BoradlessDashboard::whereIn('creator_id', $vancouver)->where('updated_at', '>', $start)->where('updated_at', '<', $end)->whereNotIn('task_status_id', [38, 36])->get();
        ...
        foreach ($boradless_data as $boradless_rec) {
            ...
            if ($boradless->task_status_id == 17) {
                $preStatus = \App\SprintTaskHistory::where('sprint_id', '=', $boradless->sprint_id)->where('status_id', '!=', '17')->orderBy('id', 'desc')->first();
                if (!empty($preStatus)) {
            ...
            if ($boradless->sprintReattempts) {
                if ($boradless->sprintReattempts->reattempts_left == 0) {
                    ...
                    $secondAttempt = \App\SprintTaskHistory::where('sprint_id', '=', $boradless->sprintReattempts->reattempt_of)->orderBy('created_at', 'ASC')->get(['status_id', \DB::raw("CONVERT_TZ(sprint__tasks_history.created_at,'UTC','America/Vancouver') as created_at")]);
                        ...
                        foreach ($secondAttempt as $secAttempt) {
                            ...
                            if ($status2) {
                                if ($eta->task_status_id == 17) {
                                    $preStatus = \App\SprintTaskHistory::where('sprint_id', '=', $eta->sprint_id)->where('status_id', '!=', '17')->orderBy('id', 'desc')->first();
                                    if (!empty($preStatus)) {
                    ...
                    if (!empty($firstSprint)) {
                        $firstAttempt = \App\SprintTaskHistory::where('sprint_id', '=', $firstSprint->reattempt_of)->orderBy('created_at', 'ASC')->get(['status_id', \DB::raw("CONVERT_TZ(sprint__tasks_history.created_at,'UTC','America/Vancouver') as created_at")]);
...
//            echo ($actual_delivery_status == 13) ? "At hub - processing"."\t" : self::$status[$actual_delivery_status] . "\t";
    ...
    /**
     * Get Vancouver Dashboard Excel OTD Report
     */
    public function vancouverDashboardExcelOtdReport($date = null , $vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
            $otd_date = date('Y-m-d');
            $otd_date = date('Y-m-d', strtotime($otd_date));
        } else {
            $otd_date = $date;
            $otd_date = date('Y-m-d', strtotime($otd_date));
        }
        ...
        $start_dt = new DateTime($otd_date . " 00:00:00", new DateTimezone('America/Vancouver'));
        $start_dt->setTimeZone(new DateTimezone('UTC'));
        $start = $start_dt->format('Y-m-d H:i:s');

        $end_dt = new DateTime($otd_date . " 23:59:59", new DateTimezone('America/Vancouver'));
        $end_dt->setTimeZone(new DateTimezone('UTC'));
        $end = $end_dt->format('Y-m-d H:i:s');
        ...
        $file_name = new \DateTime($date);
        $file_name = $file_name->format("M d, Y");
        $file_name = "Vancouver OTD Report " . $file_name . ".csv";
        $sprint_id = SprintTaskHistory::where('created_at', '>', $start)->where('created_at', '<', $end)->where('status_id', 125)->pluck('sprint_id');


        $boradless_data = BoradlessDashboard::whereIn('creator_id', $vancouver)->whereIn('sprint_id', $sprint_id)->whereNotIn('task_status_id', [38, 36])->get();
        //$boradless_data = Sprint::whereIn('id',$sprintIds)->where('deleted_at', null)->where('is_reattempt','=', 0)->get();
        // header info for browser
            ...
            if (!$boradless->sprintReattempts) {
                ...
                if (date("Y-m-d", strtotime($boradless->pickupFromStoreOtd($otd_date)->pickup)) == $otd_date) {
                    ...
                    if ($boradless->sprintReattemptsOTD) {
                        ...
                        $secondAttempt = \App\SprintTaskHistory::where('sprint_id', '=', $boradless->sprintReattemptsOTD->sprint_id)->orderBy('created_at', 'ASC')->get(['status_id', \DB::raw("CONVERT_TZ(sprint__tasks_history.created_at,'UTC','America/Vancouver') as created_at")]);
                        if (!empty($secondAttempt)) {
                        ...
                        if (!empty($firstSprint)) {
                            $firstAttempt = \App\SprintTaskHistory::where('sprint_id', '=', $firstSprint->sprint_id)->orderBy('created_at', 'ASC')->get(['status_id', \DB::raw("CONVERT_TZ(sprint__tasks_history.created_at,'UTC','America/Vancouver') as created_at")]);
                            if (!empty($firstAttempt)) {
    ...
    public function vancouverTotalCards($date, $type)
    {
        $response = [];
        ...
        $taskIds = DB::table('boradless_dashboard')->whereIn('creator_id', $vancouver)->where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)->whereNotIn('task_status_id', [38, 36])->whereNull('deleted_at')->pluck('task_id');

        $boradless = new BoradlessDashboard();
        $boradless_count = $boradless->getBoradlessCounts($taskIds, $type);
        $response['vancouver_count'] = $boradless_count;
        return $response;
    }

    public function vancouverInProgressOrders($date, $type)
    {
        $response = [];
        $date = !empty($date) ? $date : date("Y-m-d");
        ...
        $response['vancouver_inprogess_count'] = $boradless_count;
        return $response;
    }
    ...
    public function getBoradlessYesterdayOrderData($date)
    {
        ...
        $yesterday_return_orders = DB::table('boradless_dashboard')->whereIn('creator_id', $vancouver)->join('sprint_reattempts', 'boradless_dashboard.sprint_id', '=', 'sprint_reattempts.sprint_id')->where('boradless_dashboard.created_at', '>', $start)->where('boradless_dashboard.created_at', '<', $end)->whereNull('deleted_at')->count();
        $response['yesterday_return_orders'] = $yesterday_return_orders;
        return $response;
    }

    public function getVancouverCustomRouteData($date)
    {
        $response = [];
        $date = !empty($date) ? $date : date("Y-m-d");
        ...
        $custom_route = DB::table('boradless_dashboard')->whereIn('creator_id', $vancouver)->where('created_at', '>', $start)->where('created_at', '<', $end)->whereNull('deleted_at')->where('is_custom_route', 1)->count();
        $response['custom_route'] = $custom_route;
        return $response;
    }

    public function getVancouverCards(Request $request)
    {
        $type = 'all';
        return backend_view('vancouver_dashboard.vancouver_card_dashboard', compact('type'));
    }

    public function getVancouver(Request $request)
    {
        $type = 'total';
        $selectVendor = !empty($request->get('store_name')) ? $request->get('store_name') : '';
        ...
        return backend_view('vancouver_dashboard.vancouver_order_dashboard', compact('type', 'selectVendor'));
    }

    public function getVancouverData(Datatables $datatables, Request $request)
    {
        $today_date = !empty($request->get('datepicker')) ? $request->get('datepicker') : date("Y-m-d");
...
//        $boradlessVendorId = [477542];
        ...
        return $datatables->eloquent($query)->setRowId(static function ($record) {
            ...
            if ($record->picked_up_at) {
                ...
                $picked_up_at->setTimeZone(new \DateTimeZone('America/Vancouver'));
            ...
            if ($record->sorted_at) {
                ...
                $sorted_at->setTimeZone(new \DateTimeZone('America/Vancouver'));
            ...
            if ($record->delivered_at) {
                ...
                $delivered_at->setTimeZone(new \DateTimeZone('America/Vancouver'));
            ...
            return backend_view('vancouver_dashboard.order_action', compact('record'));
        })->make(true);
    }

    public function getVancouverProfile(Request $request, $id)
    {
        ...
        $data = $data['data'];
        return backend_view('vancouver_dashboard.order_profile', compact('data', 'sprintId', 'OldOrderData'));
    }
    ...
    public function getVancouverExcel($date = null, $vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradless_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)->whereIn('creator_id', $vancouver)->whereNotIn('task_status_id', [38, 36])->get();
        $boradless_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Actual Arrival @ CX', 'Vancouver tracking #', 'Status'];
        ...
        foreach ($boradless_data as $boradless) {
            ...
            $boradless_array[] = ['JoeyCo Order #' => strval($boradless->sprint_id), 'Route Number' => $boradless->route_id ? strval('R-' . $boradless->route_id . '-' . $boradless->ordinal) : '', 'Joey' => $boradless->joey_name ? strval($boradless->joey_name . ' (' . $boradless->joey_id . ')') : '', 'Customer Address' => strval($boradless->address_line_1), 'Out For Delivery' => $picked_up_at, 'Sorter Time' => $sorted_at, 'Actual Arrival @ CX' => $delivered_at, 'Vancouver tracking #' => strval(substr($boradless->tracking_id, ($pos = strrpos($boradless->tracking_id, '_')) == false ? 0 : $pos + 1)), 'Status' => $boradless->task_status_id ? strval(self::$status[$boradless->task_status_id]) : ''];
        }

        Excel::create('Vancouver Data ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('Vancouver Data');
            $excel->sheet('Vancouver Data', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }

    public function getVancouverSorter(Request $request)
    {
        $title_name = 'Vancouver';
        $type = 'sorted';
        $selectVendor = !empty($request->get('store_name')) ? $request->get('store_name') : '';
        /*$boradlessVendorId = [477542,477559,477518];
        $ctcVendors = CtcVendor::whereNotIn('vendor_id',[477340,477341,477342,477343,477344,477345,477346])->pluck('vendor_id')->toArray();
        $vendorsData = array_merge($boradlessVendorId,$ctcVendors);
        $vendorData = Vendor::whereIn('id',$vendorsData)->get();*/
        return backend_view('vancouver_dashboard.sorted_order', compact('title_name', 'type', 'selectVendor'));
    }

    public function vancouverSortedData(Datatables $datatables, Request $request)
    {
        $today_date = !empty($request->get('datepicker')) ? $request->get('datepicker') : date("Y-m-d");
        ...
        $query = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)->whereNotIn('task_status_id', [38, 36])->where(['task_status_id' => 133]);
//        $boradlessVendorId = [477542,477559,477518];
//        $ctcVendors = CtcVendor::whereNotIn('vendor_id',[477340,477341,477342,477343,477344,477345,477346])->pluck('vendor_id')->toArray();
//        $boradlessVendorIds = array_merge($boradlessVendorId,$ctcVendors);
//        if (!empty($request->get('store_name'))) {
...
//            }
        ...
        return $datatables->eloquent($query)->setRowId(static function ($record) {
            ...
            return backend_view('vancouver_dashboard.action_sorted', compact('record'));
        })->make(true);
    }
    ...
    public function vacnouversortedDetail(Request $request, $id)
    {
        ...
        return backend_view('vancouver_dashboard.vancouver_sorted_detail', compact('data', 'sprintId'));
    }

    public function vancouverSortedExcel($date = null, $vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradless_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->whereIn('creator_id', $vancouver)->where('is_custom_route', 0)->where(['task_status_id' => 133])->whereNotIn('task_status_id', [38, 36])->get();
        $boradless_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Actual Arrival @ CX', 'Vancouver tracking #', 'Status'];
        foreach ($boradless_data as $boradless) {
            $picked_up_at = '';
            $sorted_at = '';
        ...
        Excel::create(' Vancouver Data ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('Vancouver Sorted Data');
            $excel->sheet('Vancouver Sorted Data', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }

    public function getVancouverhub(Request $request)
    {
        $title_name = 'Vancouver';
        $type = 'picked';
        $selectVendor = !empty($request->get('store_name')) ? $request->get('store_name') : '';
        /* $boradlessVendorId = [477542,477559,477518];
         ...
         $vendorsData = array_merge($boradlessVendorId,$ctcVendors);
         $vendorData = Vendor::whereIn('id',$vendorsData)->get();*/
        return backend_view('vancouver_dashboard.pickup_hub', compact('title_name', 'type', 'selectVendor'));
    }

    public function vancouverPickedUpData(Datatables $datatables, Request $request)
    {
        $today_date = !empty($request->get('datepicker')) ? $request->get('datepicker') : date("Y-m-d");
        /*$boradlessVendorId = [477542,477559,477518];
        $ctcVendors = CtcVendor::whereNotIn('vendor_id',[477340,477341,477342,477343,477344,477345,477346])->pluck('vendor_id')->toArray();
        ...
        $query = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)->whereNotIn('task_status_id', [38, 36])->where(['task_status_id' => 121]);
//        $boradlessVendorId = [477542,477559,477518];
//        $ctcVendors = CtcVendor::whereNotIn('vendor_id',[477340,477341,477342,477343,477344,477345,477346])->pluck('vendor_id')->toArray();
//        $boradlessVendorIds = array_merge($boradlessVendorId,$ctcVendors);
//        if (!empty($request->get('store_name'))) {
...
//            }
        ...
        return $datatables->eloquent($query)->setRowId(static function ($record) {
            ...
            return backend_view('vancouver_dashboard.action_pickup', compact('record'));
        })->make(true);
    }

    public function vancouverpickupDetail(Request $request, $id)
    {
        $boradless_id = base64_decode($id);
        $data = $this->get_trackingorderdetails($boradless_id);
        $sprintId = $data['sprintId'];
        $data = $data['data'];
        return backend_view('vancouver_dashboard.vancouver_pickup_detail', compact('data', 'sprintId'));
    }
    ...
    public function boradlessPickedupExcel($date = null, $vendor = null)
    {
        ...
        $boradless_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)->whereIn('creator_id', $vancouver)->where(['task_status_id' => 121])->whereNotIn('task_status_id', [38, 36])->get();
        $boradless_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Actual Arrival @ CX', 'Vancouver tracking #', 'Status'];
        foreach ($boradless_data as $boradless) {
            $picked_up_at = '';
            $sorted_at = '';
        ...
        Excel::create('Vancouver Picked Up Data ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('Vancouver Picked Up Data');
            $excel->sheet('Vancouver Picked Up Data', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }

    public function getVancouverscan(Request $request)
    {
        $title_name = 'Vancouver';
        $type = 'scan';
        $selectVendor = !empty($request->get('store_name')) ? $request->get('store_name') : '';
        return backend_view('vancouver_dashboard.not_scanned_orders', compact('title_name', 'type', 'selectVendor'));
    }

    public function vancouverNotScanData(Datatables $datatables, Request $request)
    {
        $today_date = !empty($request->get('datepicker')) ? $request->get('datepicker') : date("Y-m-d");
        ...
        $query = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)->whereNotIn('task_status_id', [38, 36])->whereIn('task_status_id', [61, 13]);
//        $boradlessVendorId = [477542,477559,477518];
//        $ctcVendors = CtcVendor::whereNotIn('vendor_id',[477340,477341,477342,477343,477344,477345,477346])->pluck('vendor_id')->toArray();
//        $boradlessVendorIds = array_merge($boradlessVendorId,$ctcVendors);
//        if (!empty($request->get('store_name'))) {
...
//            }
        ...
        return $datatables->eloquent($query)->setRowId(static function ($record) {
            ...
            return backend_view('vancouver_dashboard.action_notscan', compact('record'));
        })->make(true);
    }

    public function vancouvernotscanDetail(Request $request, $id)
    {
        $boradless_id = base64_decode($id);
        $data = $this->get_trackingorderdetails($boradless_id);
        $sprintId = $data['sprintId'];
        $data = $data['data'];
        return backend_view('vancouver_dashboard.vancouver_notscan_detail', compact('data', 'sprintId'));
    }

    public function vancouverNotscanExcel($date = null, $vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradless_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->whereIn('creator_id', $vancouver)->where('is_custom_route', 0)->whereIn('task_status_id', [61, 13])->whereNotIn('task_status_id', [38, 36])->get();
        $boradless_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Actual Arrival @ CX', 'Vancouver tracking #', 'Status'];
        foreach ($boradless_data as $boradless) {
            $picked_up_at = '';
            $sorted_at = '';
            ...
            $boradless_array[] = ['JoeyCo Order #' => strval($boradless->sprint_id), 'Route Number' => $boradless->route_id ? strval('R-' . $boradless->route_id . '-' . $boradless->ordinal) : '', 'Joey' => $boradless->joey_name ? strval($boradless->joey_name . ' (' . $boradless->joey_id . ')') : '', 'Customer Address' => strval($boradless->address_line_1), 'Out For Delivery' => $picked_up_at, 'Sorter Time' => $sorted_at, 'Actual Arrival @ CX' => $delivered_at, 'Vancouver tracking #' => strval(substr($boradless->tracking_id, ($pos = strrpos($boradless->tracking_id, '_')) == false ? 0 : $pos + 1)), 'Status' => strval(self::$status[$boradless->task_status_id])];
        }
        Excel::create('Vancouver Not Scan Data ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('Vancouver Not Scan Data');
            $excel->sheet('Vancouver Not Scan Data', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }

    public function getVancouverdelivered(Request $request)
    {
        $title_name = 'Vancouver';
        $type = 'delivered';
        $selectVendor = !empty($request->get('store_name')) ? $request->get('store_name') : '';
        return backend_view('vancouver_dashboard.delivered_orders', compact('title_name', 'type', 'selectVendor'));
    }

    public function vancouverDeliveredData(Datatables $datatables, Request $request)
    {
        $today_date = !empty($request->get('datepicker')) ? $request->get('datepicker') : date("Y-m-d");
        ...
        $query = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)->whereNotIn('task_status_id', [38, 36])->whereIn('task_status_id', [17, 113, 114, 116, 117, 118, 132, 138, 139, 144]);
//        $boradlessVendorId = [477542,477559,477518];
//        $ctcVendors = CtcVendor::whereNotIn('vendor_id',[477340,477341,477342,477343,477344,477345,477346])->pluck('vendor_id')->toArray();
//        $boradlessVendorIds = array_merge($boradlessVendorId,$ctcVendors);
//        if (!empty($request->get('store_name'))) {
...
//            }
        ...
        $vancouver = [477607, 477609, 47758, 477641];
        $shipHero = [477613];
        $allVendors = array_merge($vancouver, $shipHero);
        if (!empty($request->get('store_name')) || $request->store_name != "") {
        ...
        return $datatables->eloquent($query)->setRowId(static function ($record) {
            ...
            return backend_view('vancouver_dashboard.action_delivered', compact('record'));
        })->make(true);
    }

    public function vancouverdeliveredDetail(Request $request, $id)
    {
        $boradless_id = base64_decode($id);
        $data = $this->get_trackingorderdetails($boradless_id);
        ...
        $sprintId = $data['sprintId'];
        $data = $data['data'];
        return backend_view('vancouver_dashboard.vancouver_delivered_detail', compact('data', 'sprintId'));
    }

    public function vancouverDeliveredExcel($date = null, $vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradless_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->whereIn('creator_id', $vancouver)->where('is_custom_route', 0)->whereNotIn('task_status_id', [38, 36])->whereIn('task_status_id', [17, 113, 114, 116, 117, 118, 132, 138, 139, 144])->get();
        $boradless_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Actual Arrival @ CX', 'Vancouver tracking #', 'Status'];
        foreach ($boradless_data as $boradless) {
            $picked_up_at = '';
            $sorted_at = '';
        ...
        Excel::create('Vancouver Delivered Data ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('Vancouver Delivered Data');
            $excel->sheet('Vancouver Delivered Data', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }

    public function getVancouvereturned(Request $request)
    {
        $title_name = 'Vancouver';
        $type = 'return';
        $selectVendor = !empty($request->get('store_name')) ? $request->get('store_name') : '';
        return backend_view('vancouver_dashboard.returned_orders', compact('title_name', 'type', 'selectVendor'));
    }

    public function vancouverReturnedData(Datatables $datatables, Request $request)
    {
        ...
        $query = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->whereNotIn('task_status_id', [38, 36])->whereIn('task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])->where('is_custom_route', 0);
...
//        $boradlessVendorId = [477542];
        ...
        return $datatables->eloquent($query)->setRowId(static function ($record) {
            ...
            if ($record->returned_at) {
                ...
                $returned_at->setTimeZone(new \DateTimeZone('America/Vancouver'));
            ...
            if ($record->hub_return_scan) {
                ...
                $hub_return_scan->setTimeZone(new \DateTimeZone('America/Vancouver'));
    ...
    public function vancouverreturnedDetail(Request $request, $id)
    {
        $boradless_id = base64_decode($id);
        $data = $this->get_trackingorderdetails($boradless_id);
        $sprintId = $data['sprintId'];
        $data = $data['data'];
        return backend_view('vancouver_dashboard.vancouver_returned_detail', compact('data', 'sprintId'));
    }

    public function VancouverReturnedExcel($date = null, $vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradless_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->whereIn('creator_id', $vancouver)->where('is_custom_route', 0)->whereNotIn('task_status_id', [38, 36])->whereIn('task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])->get();
        $boradless_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Joey Returned Scan', 'Hub Returned Scan', 'Vancouver tracking #', 'Status'];
        foreach ($boradless_data as $boradless) {
            $picked_up_at = '';
            $sorted_at = '';
            // $delivered_at = '';
            ...
            $boradless_array[] = ['JoeyCo Order #' => strval($boradless->sprint_id), 'Route Number' => $boradless->route_id ? strval('R-' . $boradless->route_id . '-' . $boradless->ordinal) : '', 'Joey' => $boradless->joey_name ? strval($boradless->joey_name . ' (' . $boradless->joey_id . ')') : '', 'Customer Address' => strval($boradless->address_line_1), 'Out For Delivery' => $picked_up_at, 'Sorter Time' => $sorted_at, 'Joey Returned Scan' => $returned_at, 'Hub Returned Scan' => $hub_return_scan, 'Vancouver tracking #' => strval(substr($boradless->tracking_id, ($pos = strrpos($boradless->tracking_id, '_')) == false ? 0 : $pos + 1)), 'Status' => strval(self::$status[$boradless->task_status_id])];
        }
        Excel::create('Vancouver Returned Data ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('Vancouver Returned Data');
            $excel->sheet('Vancouver Returned Data', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }

    public function getVancouverNotreturned(Request $request)
    {
        $title_name = 'Vancouver';
        $type = 'return';
        $selectVendor = !empty($request->get('store_name')) ? $request->get('store_name') : '';
        return backend_view('vancouver_dashboard.not_returned_orders', compact('title_name', 'type', 'selectVendor'));
    }

    public function vancouverNotReturnedData(Datatables $datatables, Request $request)
    {
        $today_date = !empty($request->get('datepicker')) ? $request->get('datepicker') : date("Y-m-d");
        $vancouver = [477607, 477609, 477589, 477641];
        $shipHero = [477613];
        ...
        $query = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->whereNotIn('task_status_id', [38, 36])->whereIn('task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])->where('is_custom_route', 0)->whereNull('hub_return_scan');
        if (!empty($request->get('store_name')) || $request->store_name != "") {
        ...
        return $datatables->eloquent($query)->setRowId(static function ($record) {
            ...
            ->editColumn('route_id', static function ($record) {
                ...
                return backend_view('vancouver_dashboard.action_notreturned', compact('record'));
            })->make(true);
    }

    public function vancouverNotReturnedDetail(Request $request, $id)
    {
        $boradless_id = base64_decode($id);
        $data = $this->get_trackingorderdetails($boradless_id);
        $sprintId = $data['sprintId'];
        ...
        return backend_view('vancouver_dashboard.vancouver_notreturned_detail', compact('data', 'sprintId'));
    }

    public function VancouverNotReturnedExcel($date = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradless_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->whereIn('creator_id', $vancouver)->where('is_custom_route', 0)->whereNotIn('task_status_id', [38, 36])->whereNull('hub_return_scan')->whereIn('task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])->get();
        $boradless_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Joey Returned Scan', 'Hub Returned Scan', 'Vancouver tracking #', 'Status'];
        foreach ($boradless_data as $boradless) {
            $picked_up_at = '';
            $sorted_at = '';
        ...
        Excel::create('Vancouver Returns Not Received ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('Vancouver Returns Not Received');
            $excel->sheet('Vancouver Returns Not Received', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }

    public function VancouverNotReturnedExcelTrackingIds($date = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradless_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 0)->whereIn('creator_id', $vancouver)->whereNotIn('task_status_id', [38, 36])->whereNull('hub_return_scan')->whereIn('task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])->get();
        $boradless_array[] = ['Vancouver tracking #'];
        foreach ($boradless_data as $boradless) {
            $boradless_array[] = ['Vancouver tracking #' => strval(substr($boradless->tracking_id, ($pos = strrpos($boradless->tracking_id, '_')) == false ? 0 : $pos + 1))];
        }
        Excel::create('Vancouver Tracking Not Received' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('Vancouver Tracking Not Received');
            $excel->sheet('Vancouver Tracking Not Received', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }

    public function getVancouverCustomRoute(Request $request)
    {
        $title_name = 'Vancouver';
        $type = 'custom';
        $selectVendor = !empty($request->get('store_name')) ? $request->get('store_name') : '';
        return backend_view('vancouver_dashboard.custom_route', compact('title_name', 'type', 'selectVendor'));
    }

    public function vancouverCustomRouteData(Datatables $datatables, Request $request)
    {
        ...
        $query = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->where('is_custom_route', 1);
//        $boradlessVendorId = [477542,477559,477518];
//        $ctcVendors = CtcVendor::whereNotIn('vendor_id',[477340,477341,477342,477343,477344,477345,477346])->pluck('vendor_id')->toArray();
//        $boradlessVendorIds = array_merge($boradlessVendorId,$ctcVendors);
//        if (!empty($request->get('store_name'))) {
...
//            }
        ...
        return $datatables->eloquent($query)->setRowId(static function ($record) {
            ...
            return backend_view('vancouver_dashboard.action_custom_route', compact('record'));
        })->make(true);
    }

    public function vancouverCustomRouteDetail(Request $request, $id)
    {
        $boradless_id = base64_decode($id);
        $data = $this->get_trackingorderdetails($boradless_id);
        $sprintId = $data['sprintId'];
        ...
        return backend_view('vancouver_dashboard.vancouver_custom_route_detail', compact('data', 'sprintId'));
    }

    public function vancouverCustomRouteExcel($date = null, $vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradless_data = BoradlessDashboard::where('created_at', '>', $start)->where('created_at', '<', $end)->whereIn('creator_id', $vancouver)->where('is_custom_route', 1)->get();
        $boradless_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Actual Arrival @ CX', 'Vancouver tracking #', 'Status'];
        foreach ($boradless_data as $boradless) {
            $picked_up_at = '';
            $sorted_at = '';
        ...
        Excel::create('Vancouver Custom Route Data ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('Vancouver Custom Route Data');
            $excel->sheet('Vancouver Custom Route Data', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }
    ...
    public function getRouteinfo(Request $request)
    {
        ...
        $boradless_info = JoeyRoutes::join('joey_route_locations', 'joey_routes.id', '=', 'joey_route_locations.route_id')->join('sprint__tasks','joey_route_locations.task_id', '=', 'sprint__tasks.id')->where('joey_routes.date', 'like', $date . "%")->where('joey_routes.hub', 129)->whereNotIn('sprint__tasks.status_id', [36])->where('joey_routes.deleted_at', null)->where('joey_route_locations.deleted_at', null)->orderBy('joey_routes.id', 'ASC')->groupBy('joey_routes.id')->select('joey_routes.*')->get();

        $boradlessVendorIds = [477607, 477609, 477613, 477589, 477641];
        ...
        if ($boradless_info->isEmpty()) {
            ...
            return backend_view('vancouver_dashboard.vancouver_route_info', compact('boradless_info', 'flagCategories', 'boradlessVendorIds', 'counts', 'order_type'));
        } else {
    ...
    /**
     * Get Vancouver Route Info excel report
     */
    public function vancouverRouteinfoExcel($date = null)
    {
        ...
        //JoeyRoutes::where(\DB::raw("CONVERT_TZ(date,'UTC','America/Vancouver')"),'like',$date."%")->where('hub',17)->get();
    ...
    /**
     * Get Vancouver Hub Route edit
     */
    public function vancouverHubRouteEdit(Request $request, $routeId, $hubId)
    {

        $tracking_id = null;

        $status = null;
        $route = JoeyRouteLocations::join('sprint__tasks', 'joey_route_locations.task_id', '=', 'sprint__tasks.id')->leftJoin('merchantids', 'merchantids.task_id', '=', 'sprint__tasks.id')->join('locations', 'location_id', '=', 'locations.id')->join('sprint__sprints', 'sprint_id', '=', 'sprint__sprints.id')->leftJoin('sprint__contacts', 'sprint__contacts.id', '=', 'sprint__tasks.contact_id')->whereNull('sprint__sprints.deleted_at')->whereNotIn('sprint__tasks.status_id', [36])->where('route_id', '=', $routeId)->whereNull('joey_route_locations.deleted_at')->whereNotNull('merchantids.tracking_id')->orderBy('joey_route_locations.ordinal', 'asc');
        ...
        return backend_view('vancouver_dashboard.edit-hub-route', ['route' => $route, 'hub_id' => $hubId, 'tracking_id' => $tracking_id, 'status_select' => $status, 'boradlessVendorIds' => $boradlessVendorIds, 'joey' => $joey]);
    }
    ...
    public function flagHistoryModelHtmlRender(Request $request)
    {
        ...
        $html = view('backend.vancouver_dashboard.sub-views.ajax-render-view-edit-hub-route-flag-model', compact('joey_flags_history', 'flagCategories', 'request_data'))->render();

        return response()->json(['status' => true, 'html' => $html]);
    }

    /**
     * Get Vancouver Tracking Order Detail
     */
    public function getVancouvertrackingorderdetails($sprintId)
    {
        $result = Sprint::join('sprint__tasks', 'sprint_id', '=', 'sprint__sprints.id')->leftJoin('merchantids', 'merchantids.task_id', '=', 'sprint__tasks.id')->leftJoin('joey_route_locations', 'joey_route_locations.task_id', '=', 'sprint__tasks.id')->leftJoin('joey_routes', 'joey_routes.id', '=', 'joey_route_locations.route_id')->leftJoin('joeys', 'joeys.id', '=', 'joey_routes.joey_id')->join('locations', 'sprint__tasks.location_id', '=', 'locations.id')->join('sprint__contacts', 'contact_id', '=', 'sprint__contacts.id')->leftJoin('vendors', 'creator_id', '=', 'vendors.id')->where('sprint__tasks.sprint_id', '=', $sprintId)
            //->whereNull('joey_route_locations.deleted_at')
            ->orderBy('ordinal', 'DESC')->take(1)->get(array('sprint__tasks.*', 'joey_routes.id as route_id', \DB::raw("CONVERT_TZ(joey_routes.date,'UTC','America/Vancouver') as route_date"), 'locations.address', 'locations.suite', 'locations.postal_code', 'sprint__contacts.name', 'sprint__contacts.phone', 'sprint__contacts.email', 'joeys.first_name as joey_firstname', 'joeys.id as joey_id', 'joeys.last_name as joey_lastname', 'vendors.first_name as merchant_firstname', 'vendors.last_name as merchant_lastname', 'vendors.name as business_name', 'merchantids.scheduled_duetime', 'joeys.id as joey_id', 'merchantids.tracking_id', 'joeys.phone as joey_contact', 'joey_route_locations.ordinal as stop_number'));
        ...
        return backend_view('vancouver_dashboard.orderdetailswtracknigid', ['data' => $data, 'sprintId' => $sprintId]);
    }

    /**
     * Get Vancouver Reporting
     */
    public function getVancouverReporting(Request $request)
    {
        ...
        if ($interval->days > 30) {
            ...
            return redirect('vancouver/reporting');
        }
        $all_dates = array();
        ...
        $start_dt = new DateTime($from_date . " 00:00:00", new DateTimezone('America/Vancouver'));
        $start_dt->setTimeZone(new DateTimezone('UTC'));
        $start = $start_dt->format('Y-m-d H:i:s');

        $end_dt = new DateTime($to_date . " 23:59:59", new DateTimezone('America/Vancouver'));
        $end_dt->setTimeZone(new DateTimezone('UTC'));
        $end = $end_dt->format('Y-m-d H:i:s');
        ...
        if (!empty($request->get('creator_id')) || $request->creator_id != "") {
            if ($request->get('creator_id') == 'walmart_vendors') {
                $sprint_ids = $sprint_ids->whereIn('creator_id', $vancouver);
        ...
        foreach ($all_dates as $range_date) {
            ...
            $start_dt = new DateTime($range_date . " 00:00:00", new DateTimezone('America/Vancouver'));
            $start_dt->setTimeZone(new DateTimezone('UTC'));
            $start = $start_dt->format('Y-m-d H:i:s');

            $end_dt = new DateTime($range_date . " 23:59:59", new DateTimezone('America/Vancouver'));
            $end_dt->setTimeZone(new DateTimezone('UTC'));
            $end = $end_dt->format('Y-m-d H:i:s');
        ...
        return backend_view('vancouver_dashboard.reporting.vancouver_reporting', compact('boradless_count', 'boradless_range_count', 'city', 'selectVendor'));
    }

    /**
     * Get Vancouver Route Info
     */
    public function getVancouverReportingData(Datatables $datatables, Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $end_dt = new DateTime($from_date . " 23:59:59", new DateTimezone('America/Vancouver'));
            $end_dt->setTimeZone(new DateTimezone('UTC'));
            $end = $end_dt->format('Y-m-d H:i:s');
            ...
            //$query = DB::table('boradless_dashboard')->whereBetween(\DB::raw("CONVERT_TZ(updated_at,'UTC','America/Vancouver')"), [$from_date, $to_date]);
            $sprint = new BoradlessDashboard();
            // useing fillters
    ...
    /**
     * Get Vancouver OTD Graph
     */
    public function statistics_otd_index(Request $request)
    {
        return backend_view('vancouver_dashboard.otd.statistics_otd_dashboard');
    }

    /**
     * Get Day Vancouver OTD Graph
     */
    public function ajax_render_vancouver_otd_day(Request $request)
    {
        $date = date("Y-m-d");
        $sprint = new Sprint();
        ...
        $vancouver = [477607, 477609, 477613, 477589, 477641];

        $query = BoradlessDashboard::whereIn('creator_id', $vancouver)->where('created_at', '>', $start)->where('created_at', '<', $end)->whereNotIn('task_status_id', [38, 36])->whereIn('task_status_id', $sprint->getStatusCodes('competed'))->get();
    ...
    /**
     * Get Week Vancouver OTD Graph
     */
    public function ajax_render_vancouver_otd_week(Request $request)
    {
        $date = date("Y-m-d");
        $sprint = new Sprint();
        ...
        $query = BoradlessDashboard::whereIn('creator_id', $vancouver)->whereIn('task_status_id', $sprint->getStatusCodes('competed'))->whereNotIn('task_status_id', [38, 36])->whereBetween(\DB::raw("CONVERT_TZ(created_at,'UTC','America/Vancouver')"), [date('y-m-d', strtotime('-6 day', strtotime($date))) . ' 20:00:00', $date . " 19:59:59"])->get();
    ...
    /**
     * Get Month Vancouver OTD Graph
     */
    public function ajax_render_vancouver_otd_month(Request $request)
    {
        $date = date("Y-m-d");
        $sprint = new Sprint();
        $vancouver = [477607, 477609, 477613, 477589, 477641];

        $query = BoradlessDashboard::whereIn('creator_id', $vancouver)->whereIn('task_status_id', $sprint->getStatusCodes('competed'))->whereNotIn('task_status_id', [38, 36])->whereBetween(\DB::raw("CONVERT_TZ(created_at,'UTC','America/Vancouver')"), [date('y-m-d', strtotime('-1 month', strtotime($date))) . ' 20:00:00', $date . " 19:59:59"])->get();
        //$query = Sprint::whereIn('id', $sprintIds)->where('deleted_at', null)->where('is_reattempt','=', 0)->get();
    ...
    public function getVancouverAtHub(Request $request)
    {
        $title_name = 'Vancouver';
        $type = $request->type;
        return backend_view('vancouver_dashboard.reporting.summary_orders', compact('title_name', 'type'));
    }

    public function getVancouverAtStore(Request $request)
    {
        $title_name = 'Vancouver';
        $type = $request->type;
        return backend_view('vancouver_dashboard.reporting.summary_orders', compact('title_name', 'type'));
    ...
    public function getVancouverSortedAt(Request $request)
    {
        $title_name = 'Vancouver';
        $type = $request->type;
        return backend_view('vancouver_dashboard.reporting.summary_orders', compact('title_name', 'type'));
    ...
    public function getVancouverDeliveredOrders(Request $request)
    {
        $title_name = 'Vancouver';
        $type = $request->type;
        return backend_view('vancouver_dashboard.reporting.summary_orders', compact('title_name', 'type'));
    ...
    public function vancouverAtStoreData(Datatables $datatables, Request $request)
    {
    ...
    public function getVancouverPickedUpFromStore(Request $request)
    {
        $title_name = 'Vancouver';
        $type = $request->type;
        return backend_view('vancouver_dashboard.reporting.summary_orders', compact('title_name', 'type'));
