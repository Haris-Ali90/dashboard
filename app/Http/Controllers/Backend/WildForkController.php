...
class WildForkController extends BackendController
{
    ...
    public function get_trackingorderdetails($sprintId)
    {
        ...
        $result = Sprint::join('sprint__tasks', 'sprint_id', '=', 'sprint__sprints.id')
            ...
            , 'joeys.id as joey_id', 'merchantids.tracking_id', 'joeys.phone as joey_contact', 'joey_route_locations.ordinal as stop_number', 'merchantids.merchant_order_num', 'merchantids.address_line2', 'sprint__sprints.creator_id', 'sprint__sprints.tip'));
    ...
    //WildFork Function
    public function getWildFork(Request $request)
    {
        ...
        return backend_view('wildforkDashboard.wildfork_dashboard', compact('city', 'status_code'));
    }

    public function WildForkData(Datatables $datatables, Request $request)
    {
        ...
        $ctcVendorIds = [];
        if ($city_data == 'all') {
            $ctcVendorIds = [477625, 477633, 477635];
        ...
        if (!empty($request->get('tracking_id'))) {
            $query = BoradlessDashboard::where('sprint_id', $sprintId)->whereNull('deleted_at');
        }
            ...
            $query = BoradlessDashboard::whereIn('creator_id', $ctcVendorIds)->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
                ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->whereNotIn('task_status_id', [38, 36]);
        }
        ...
        return $datatables->eloquent($query)->setRowId(static function ($record) {
            ...
            ->addColumn('tip', static function ($record) {

                if (isset($record->Sprint))
                {
                    return $record->Sprint->tip;
                }
                else
                {
                    return '';
            ...
            ->addColumn('eta_pickup', static function ($record) {

                if (isset($record->eta_time))
                {
                    return date("Y-m-d H:i:s", substr($record->eta_time, 0, 10));
                }
                else
                {
                    return '';
            ...
            return backend_view('wildforkDashboard.wildfork-action', compact('record'));
        })->make(true);

    }

    public function WildForkProfile(Request $request, $id)
    {
        $commerce_data = $this->get_trackingorderdetails($id);
        $sprintId = $commerce_data['sprintId'];
        $data = $commerce_data['data'];
        return backend_view('wildforkDashboard.wildfork_profile', compact('data', 'sprintId'));
    }

    public function getWildForkDashboardExcel($date = null,$vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradlessVendorIds = [];
        if ($vendor == 'all') {
            $boradlessVendorIds = [477625, 477633, 477635];
        ...
        $boradless_data = BoradlessDashboard::where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->where('is_custom_route', 0)->whereIn('creator_id', $boradlessVendorIds)->whereNotIn('task_status_id', [38, 36])->get();
    ...
    //Wildfork E-Commerce Dashboard Cards
    public function getwildforkCards(Request $request)
    {
        $type = 'all';
        return backend_view('wildforkDashboard.wildfork_card_dashboard', compact( 'type'));
    }
    public function wildforkEcommerceTotalCards($date, $type, $vendor_id = null)
    {
        $response = [];
        $date = !empty($date) ? $date : date("Y-m-d");

        $allVendors = [477625, 477633, 477635];
        ...
        $taskIds = DB::table('boradless_dashboard')->whereIn('creator_id', $boradlessVendorIds)->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)
        ...
        $boradless_count = $boradless->getWildforkECommerceCounts($taskIds, $type);
        $response['boradless_count'] = $boradless_count;

        return $response;
    }
    public function wildforkTotalCards($date, $type)
    {
        $response = [];
        $date = !empty($date) ? $date : date("Y-m-d");

        $wildfork = [477625, 477633, 477635];
        ...
        $taskIds = \Illuminate\Support\Facades\DB::table('boradless_dashboard')->whereIn('creator_id', $wildfork)->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)
            ->where('is_custom_route', 0)->whereNotIn('task_status_id', [38, 36])->whereNull('deleted_at')->pluck('task_id');
    ...
    public function getwildforkCustomRouteData($date)
    {
        $response = [];
        $date = !empty($date) ? $date : date("Y-m-d");
        ...
        $custom_route = \Illuminate\Support\Facades\DB::table('boradless_dashboard')->whereIn('creator_id', $wildfork)->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->whereNull('deleted_at')->where('is_custom_route', 1)->count();
        $response['custom_route'] = $custom_route;
        return $response;
    }
    public function wildforkInProgressOrders($date, $type)
    {
        $response = [];
        $date = !empty($date) ? $date : date("Y-m-d");
    ...
    //Wildfork E-Commerce Dashboard Function
    public function getWildforkEcommerceDashboard(Request $request)
    {
        $type = 'total';
        $city = !empty($request->get('store_name')) ? $request->get('store_name') : 'all';
        return backend_view('wildforkDashboard.wildfork_order_dashboard', compact( 'type','city'));
    }
    //Wildfork E-Commerce Dashboard Data
    public function getWildforkEcommerceDashboardData(Datatables $datatables, Request $request)
    {
        $today_date = !empty($request->get('datepicker')) ? $request->get('datepicker') : date("Y-m-d");
        ...
        $query = BoradlessDashboard::where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->whereIn('creator_id', $ctcVendorIds)->where('is_custom_route', 0)->whereNotIn('task_status_id', [38, 36]);
        ...
        return $datatables->eloquent($query)
            ...
            ->editColumn('tip', static function ($record) {
            ...
            ->addColumn('action', static function ($record) {
                return backend_view('wildforkDashboard.order_action', compact('record'));
            })
            ->make(true);
    }
    public function getWildforkExcel($date = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradlessVendorIds = [477625, 477633, 477635];
        $boradless_data = BoradlessDashboard::where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->where('is_custom_route', 0)->whereIn('creator_id', $boradlessVendorIds)->whereNotIn('task_status_id', [38, 36])->orderBy('sprint_id','desc')->get();
        $boradless_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Actual Arrival @ CX', 'tracking #', 'Status'];
        ...
        Excel::create('WildFork Data ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('WildFork Data');
            $excel->sheet('WildFork Data', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }
    public function getWildforkEcommerceDashboardExcel($date = null,$vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
    ...
    public function getWildforkEcommerceSorter(Request $request)
    {
        $title_name = 'WildFork E-commerce';
        $type = 'sorted';
        $city = !empty($request->get('store_name')) ? $request->get('store_name') : 'all';

        return backend_view('wildforkDashboard.sorted_order', compact('title_name',  'type','city'));
    }
    public function wildforkEcommerceSortedData(Datatables $datatables, Request $request)
    {
        $today_date = !empty($request->get('datepicker')) ? $request->get('datepicker') : date("Y-m-d");
...
//            }
        return $datatables->eloquent($query)
            ...
            ->addColumn('action', static function ($record) {
                return backend_view('wildforkDashboard.action_sorted', compact('record'));
            })
            ->make(true);
    }
    public function wildforkEcommerceSortedExcel($date = null,$vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradless_data = BoradlessDashboard::where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->whereIn('creator_id', $boradlessVendorIds)->where('is_custom_route', 0)->where(['task_status_id' => 133])->whereNotIn('task_status_id', [38, 36])->orderBy('sprint_id','desc')->get();
        $boradless_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Actual Arrival @ CX', 'Tracking #', 'Status'];
        foreach ($boradless_data as $boradless) {
            $picked_up_at = '';
            $sorted_at = '';
        ...
        Excel::create('WildFork Sorted Data ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('WildFork Sorted Data');
            $excel->sheet('WildFork Sorted Data', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }
    public function getWildforkEcommercehub(Request $request)
    {
        $title_name = 'WildFork E-commerce';
        $type = 'picked';
        $city = !empty($request->get('store_name')) ? $request->get('store_name') : 'all';
        return backend_view('wildforkDashboard.pickup_hub', compact('title_name',  'type','city'));
    }
    public function wildforkEcommercePickedUpData(Datatables $datatables, Request $request)
    {
        $today_date = !empty($request->get('datepicker')) ? $request->get('datepicker') : date("Y-m-d");
        /*$boradlessVendorId = [477542,477559,477518];
        $ctcVendors = CtcVendor::whereNotIn('vendor_id',[477340,477341,477342,477343,477344,477345,477346])->pluck('vendor_id')->toArray();
       ...
       /*elseif ($city_data == 'ottawa') {
        ...
        return $datatables->eloquent($query)
            ...
            ->addColumn('action', static function ($record) {
                return backend_view('wildforkDashboard.action_pickup', compact('record'));
            })
            ->make(true);
    }
    public function wildforkEcommercePickedupExcel($date = null,$vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradless_data = BoradlessDashboard::where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->where('is_custom_route', 0)->whereIn('creator_id', $boradlessVendorIds)->where(['task_status_id' => 121])->whereNotIn('task_status_id', [38, 36])->orderBy('sprint_id','desc')->get();
        $boradless_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Actual Arrival @ CX', 'Tracking #', 'Status'];
        foreach ($boradless_data as $boradless) {
            $picked_up_at = '';
            $sorted_at = '';
        ...
        Excel::create('WildFork Picked Up Data ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('WildFork Picked Up Data');
            $excel->sheet('WildFork Picked Up Data', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }
    public function getWildforkEcommercescan(Request $request)
    {
        $title_name = 'WildFork E-commerce';
        $type = 'scan';
        $city = !empty($request->get('store_name')) ? $request->get('store_name') : 'all';
        return backend_view('wildforkDashboard.not_scanned_orders', compact('title_name', 'type','city'));
    }
    public function wildforkEcommerceNotScanData(Datatables $datatables, Request $request)
    {
        $today_date = !empty($request->get('datepicker')) ? $request->get('datepicker') : date("Y-m-d");
        ...
        return $datatables->eloquent($query)
            ...
            ->addColumn('action', static function ($record) {
                return backend_view('wildforkDashboard.action_notscan', compact('record'));
            })
            ->make(true);
    }
    public function wildforkEcommercescanExcel($date = null,$vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradless_data = BoradlessDashboard::where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->whereIn('creator_id', $boradlessVendorIds)->where('is_custom_route', 0)->whereIn('task_status_id', [61, 13])->whereNotIn('task_status_id', [38, 36])->orderBy('sprint_id','desc')->get();
        $boradless_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Actual Arrival @ CX', 'Tracking #', 'Status'];
        foreach ($boradless_data as $boradless) {
            $picked_up_at = '';
            $sorted_at = '';
        ...
        Excel::create('WildFork Not Scan Data ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('WildFork Not Scan Data');
            $excel->sheet('WildFork Not Scan Data', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }
    public function getWildforkEcommercedelivered(Request $request)
    {
        $title_name = 'WildFork E-commerce';
        $type = 'delivered';
        $city = !empty($request->get('store_name')) ? $request->get('store_name') : 'all';
        return backend_view('wildforkDashboard.delivered_orders', compact('title_name',  'type','city'));
    }
    public function wildforkEcommerceDeliveredData(Datatables $datatables, Request $request)
    {
        $today_date = !empty($request->get('datepicker')) ? $request->get('datepicker') : date("Y-m-d");
        ...
        return $datatables->eloquent($query)
            ...
            ->addColumn('action', static function ($record) {
                return backend_view('wildforkDashboard.action_delivered', compact('record'));
            })
            ->make(true);
    }
    public function wildforkEcommerceDeliveredExcel($date = null,$vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradless_data = BoradlessDashboard::where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->whereIn('creator_id', $boradlessVendorIds)->where('is_custom_route', 0)->whereNotIn('task_status_id', [38, 36])->orderBy('sprint_id','desc')
        ...
        Excel::create('WildFork Delivered Data ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('WildFork Delivered Data');
            $excel->sheet('WildFork Delivered Data', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }
    public function getWildforkEcommercereturned(Request $request)
    {
        $title_name = 'WildFork E-commerce';
        $type = 'return';
        $city = !empty($request->get('store_name')) ? $request->get('store_name') : 'all';
        return backend_view('wildforkDashboard.returned_orders', compact('title_name',  'type','city'));
    }
    public function wildforkEcommerceReturnedData(Datatables $datatables, Request $request)
    {
        ...
        $query = BoradlessDashboard::where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->whereIn('creator_id', $ctcVendorIds)->whereNotIn('task_status_id', [38, 36])
        ...
        return $datatables->eloquent($query)
            ...
            ->addColumn('action', static function ($record) {
                return backend_view('wildforkDashboard.action_returned', compact('record'));
            })
            ->make(true);
    }
    public function wildforkEcommerceReturnedExcel($date = null,$vendor = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradless_data = BoradlessDashboard::where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->whereIn('creator_id', $boradlessVendorIds)->where('is_custom_route', 0)->whereNotIn('task_status_id', [38, 36])->whereIn('task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])->orderBy('sprint_id','desc')->get();
        ...
        Excel::create('WildFork Returned Data ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('WildFork Returned Data');
            $excel->sheet('WildFork Returned Data', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }
    public function getWildforkEcommerceNotreturned(Request $request)
    {
        $title_name = 'WildFork E-commerce';
        $type = 'return';
        return backend_view('wildforkDashboard.not_returned_orders', compact('title_name',  'type'));
    }
    public function wildforkEcommerceNotReturnedData(Datatables $datatables, Request $request)
    {
        $today_date = !empty($request->get('datepicker')) ? $request->get('datepicker') : date("Y-m-d");
        $boradlessVendorIds = [477625, 477635, 477633];
        ...
        $query = BoradlessDashboard::where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->whereIn('creator_id', $boradlessVendorIds)->whereNotIn('task_status_id', [38, 36])
            ->whereIn('task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])
            ->where('is_custom_route', 0)
        ...
        return $datatables->eloquent($query)
            ...
            ->addColumn('action', static function ($record) {
                return backend_view('wildforkDashboard.action_notreturned', compact('record'));
            })
            ->make(true);
    }
    public function wildforkEcommerceNotReturnedExcel($date = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradlessVendorIds = [477625, 477633, 477635];
        $boradless_data = BoradlessDashboard::where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->whereIn('creator_id', $boradlessVendorIds)->where('is_custom_route', 0)->whereNotIn('task_status_id', [38, 36])->whereNull('hub_return_scan')->whereIn('task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])->orderBy('sprint_id','desc')->get();
        $boradless_array[] = ['JoeyCo Order #', 'Route Number', 'Joey', 'Customer Address', 'Out For Delivery', 'Sorted Time', 'Joey Returned Scan', 'Hub Returned Scan', 'Tracking #', 'Status'];
        foreach ($boradless_data as $boradless) {
            $picked_up_at = '';
            $sorted_at = '';
        ...
        Excel::create('WildFork Returns Not Received ' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('WildFork Returns Not Received');
            $excel->sheet('WildFork Returns Not Received', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }
    public function wildforkEcommerceNotReturnedExcelTrackingIds($date = null)
    {
        if ($date == null) {
            $date = date('Y-m-d');
        }
        ...
        $boradlessVendorIds = [477625, 477633, 477635];
        $boradless_data = BoradlessDashboard::where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->where('is_custom_route', 0)->whereIn('creator_id', $boradlessVendorIds)->whereNotIn('task_status_id', [38, 36])->whereNull('hub_return_scan')->whereIn('task_status_id', [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 131, 135, 136, 143])->orderBy('sprint_id','desc')->get();
        ...
        Excel::create('WildFork Tracking Not Received' . $date . '', function ($excel) use ($boradless_array) {
            $excel->setTitle('WildFork Tracking Not Received');
            $excel->sheet('WildFork Tracking Not Received', function ($sheet) use ($boradless_array) {
                $sheet->fromArray($boradless_array, null, 'A1', false, false);
            });
        })->download('csv');
    }
    public function getWildforkEcommerceReporting(Request $request)
    {
        ...
        if ($interval->days > 14) {
            ...
            return redirect('wildfork/e-commerce/reporting');
        }
        $all_dates = array();
        ...
        $sprint_ids = BoradlessDashboard::where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->orderBy('sprint_id','desc')
            ->whereNotIn('task_status_id', [38, 36]);
        $city_data = $request->get('creator_id');
        $ctcVendorIds = [];
        if ($city_data == 'all') {
            $sprint_ids = $sprint_ids->whereIn('creator_id', [477625, 477633, 477635]);
        } /*elseif ($city_data == 'ottawa') {
        ...
        return backend_view('wildforkDashboard.reporting.wildfork_reporting', compact('boradless_count',
            'boradless_range_count',
            'city'
        ));
    }
    public function getWildforkEcommerceReportingData(Datatables $datatables, Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($storeName == 'all') {
                $query = $query->whereIn('creator_id', [477625, 477633, 477635]);
            ...
            return $datatables->eloquent($query)
                ...
                ->addColumn('action', static function ($record) {
                    return backend_view('wildforkDashboard.action', compact('record'));
                })
                ->addColumn('tip', static function ($record) {
    ...
    {
        return backend_view('wildforkDashboard.otd.statistics_otd_dashboard');
    }
    ...
    public function ajax_render_boradless_otd_day(Request $request)
    {
        ...
        $boradlessVendorIds = [477625, 477633, 477635];
        $query = BoradlessDashboard::where('creator_id', $boradlessVendorIds)->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '>' , $start)
            ->where(\DB::raw("FROM_UNIXTIME(eta_time)"), '<' , $end)->whereNotIn('task_status_id', [38, 36])
    ...
    public function ajax_render_boradless_otd_week(Request $request)
    {
        ...
        $query = BoradlessDashboard::where('creator_id', $boradlessVendorIds)->whereIn('task_status_id', $sprint->getStatusCodes('competed'))->whereNotIn('task_status_id', [38, 36])->orderBy('sprint_id','desc')
            ->whereBetween(\DB::raw("CONVERT_TZ(FROM_UNIXTIME(eta_time),'UTC','America/Toronto')"), [date('y-m-d', strtotime('-6 day', strtotime($date))) . ' 20:00:00', $date . " 19:59:59"])->get();
        //$query = Sprint::whereIn('id', $sprintIds)->where('deleted_at', null)->where('is_reattempt','=', 0)->get();
    ...
    public function ajax_render_boradless_otd_month(Request $request)
    {
        ...
        $query = BoradlessDashboard::where('creator_id', $boradlessVendorIds)->whereIn('task_status_id', $sprint->getStatusCodes('competed'))->whereNotIn('task_status_id', [38, 36])->orderBy('sprint_id','desc')
            ->whereBetween(\DB::raw("CONVERT_TZ(FROM_UNIXTIME(eta_time),'UTC','America/Toronto')"), [date('y-m-d', strtotime('-1 month', strtotime($date))) . ' 20:00:00', $date . " 19:59:59"])->get();
        //$query = Sprint::whereIn('id', $sprintIds)->where('deleted_at', null)->where('is_reattempt','=', 0)->get();
    ...
    public function getWildforkEcommerceDashboardProfile(Request $request, $id)
    {
        ...
        return backend_view('wildforkDashboard.order_profile', compact('data', 'sprintId'));
    }
    public function getWildforkEcommerceSortedProfile(Request $request, $id)
    {
        ...
        return backend_view('wildforkDashboard.wildfork_ecommerce_sorted_detail', compact('data', 'sprintId'));
    }
    public function wildforkEcommercepickupDetail(Request $request, $id)
    {
        $boradless_id = base64_decode($id);
        $data = $this->get_trackingorderdetails($boradless_id);
        $sprintId = $data['sprintId'];
        ...
        return backend_view('wildforkDashboard.wildfork_ecommerce_pickup_detail', compact('data', 'sprintId'));
    }
    public function wildforkEcommercenotscanDetail(Request $request, $id)
    {
        $boradless_id = base64_decode($id);
        $data = $this->get_trackingorderdetails($boradless_id);
        $sprintId = $data['sprintId'];
        $data = $data['data'];
        return backend_view('wildforkDashboard.wildfork_ecommerce_notscan_detail', compact('data', 'sprintId'));
    }
    public function wildforkEcommercedeliveredDetail(Request $request, $id)
    {
        $boradless_id = base64_decode($id);
        $data = $this->get_trackingorderdetails($boradless_id);
        ...
        $sprintId = $data['sprintId'];
        $data = $data['data'];
        return backend_view('wildforkDashboard.wildfork_delivered_detail', compact('data', 'sprintId'));
    }
    public function wildforkEcommercereturnedDetail(Request $request, $id)
    {
        $boradless_id = base64_decode($id);
        $data = $this->get_trackingorderdetails($boradless_id);
        $sprintId = $data['sprintId'];
        $data = $data['data'];
        return backend_view('wildforkDashboard.wildfork_returned_detail', compact('data', 'sprintId'));
    }
    public function wildforkEcommerceNotReturnedDetail(Request $request, $id)
    {
        $boradless_id = base64_decode($id);
        $data = $this->get_trackingorderdetails($boradless_id);
        $sprintId = $data['sprintId'];
        ...
        return backend_view('wildforkDashboard.wildfork_notreturned_detail', compact('data', 'sprintId'));
    }
    public function getWildforkEcommerceCards(Request $request)
    {
        $type = 'all';
        return backend_view('wildforkDashboard.wildfork_card_dashboard', compact( 'type'));
    }
